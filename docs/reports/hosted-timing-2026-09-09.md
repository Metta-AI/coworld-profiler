...
- Extend the pod watcher to player pods so `container.start` spans cover the 4 to 24 s per slot that dominates `first_step_s`. Take container and init-container timing from the pod status the watcher already stores (`initContainerStatuses` and `containerStatuses` start and finish times) rather than from kubelet Events, whose capture depends on the watcher staying up; if pull spans stay, filter the event stream server-side by reason, persist its position, and parse the pull time from the `Pulled` message so cached pulls under `imagePullPolicy: Always` stop reading as zero-length misses.
