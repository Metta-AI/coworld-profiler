...
tools/analyze_replay.py      print a summary from a replay or a results file
tests/                       pytest
```
...
uv sync --group dev
uv run pytest
uv run coworld build --version 0.1.0          # from the metta checkout's venv, or any venv with coworld
uv run coworld run-episode dist/coworld_manifest.json
uv run coworld run-episode dist/coworld_manifest.json --variant fanout-8p
```
...
uv run coworld upload-policy coworld-profiler:latest --name profiler-echo --run python --run -m --run profiler.player.player --run --policy --run echo
uv run coworld xp-request create body.json    # coworld_id + variant_id + roster of policy_refs
```
